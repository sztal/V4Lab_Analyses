...
# Introduction
This report presents basic analysis of the answers to the KNOWLEDGE questions the in CZ sample. Firstly, it addresses the problem of the questions' difficulty levels (defined as the fraction of the respondents that failed to answer a particular question correctly). Secondly, it examines the distribution of the KNOWLEDGE raw scores. Here a raw score is defiened as the sum of the correct answers given by a respondent. This two subanalyses will yield some important insight into the level of economic/financial knowledge of the Czech respondents.
...
* **Easy items:** k21, k29, k16, k23, k30, k26, k18, k24, k12, k15, k28, k3, k25, k17, k13, k8, k27, k4
* **In-between items:** k2, k11, k14, k7
* **Tricky items:** k20, k5, k6, k1, k19, k10, k22
...
The distribution is not very clearly symmetric and rather far from normal, what implies that some additional analyses and modelling may be necessary to scale this set of items appropriately. A formal check of normality (Shapiro-Wilk test) yielded negative results, what confirms an unusual and a little 'ragged' character of the distribution. Nevertheless graphical inspection of the distribution also confirms what was already suggested by the distribution of the items' difficulty levels - the items are of various difficulty levels and by this virtue they may be useful for the measurment of economic/financial knowledge.
...
At the end we provide a numerical summary of the raw scores distribution. It shows that the raw scores have a moderate range of 14 points (out of 30 possible) with min at 10 and max at 24. Moreover, it also shows that the distribution is slightly shifted to the right what is implied by median at 18 and mean at 17.88, so on average respondents were slightly better than guessing.

```{r, echo=FALSE}
summary(rawscores)
```

The shift of the mean to 17.88 from 15 (the expected score of a guessing person) was proved to be statistically significant (95% confidence interval of the mean is: [17.53 - 18.23]). It means that on average respondents knew correct answers to about 6 questions (6 + 24/2 = 6 + 12 = 18). This calculation is of course based on an assumption that all 24 remaining questions were guessed.
