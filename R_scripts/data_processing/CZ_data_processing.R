...
###################################
### CZ_main dataset preparation ###
###################################

### This script transforms Czech rawdata to a more tidy and usable format
### that is also free of non_ASCII characters.
### Load data
data <- read.csv(normalizePath("./Data/RawData/CZ_rawdata.csv"), sep=";")
...
sp[sp == ""] <- NA
sp[grep("admini.*publ.*reg.*roz", sp, ignore.case = TRUE, perl = TRUE)] <- "Administration"
sp[grep("BANK", sp, ignore.case = TRUE, perl = TRUE)] <- "Finance/Actuarial"
...
sp[grep("HANDEL", sp, ignore.case = TRUE, perl = TRUE)] <- "Bussiness"
sp[grep("INF", sp, ignore.case = TRUE, perl = TRUE)] <- "Math/CS"
sp[grep("KOMUNIKA", sp, ignore.case = TRUE, perl = TRUE)] <- "PR"
...
sp[grep("PRAWO$", sp, ignore.case = TRUE, perl = TRUE)] <- "Law"
sp[grep("RELIGION", sp, ignore.case = TRUE, perl = TRUE)] <- "Religion_Studies"
sp[grep("REST ", sp, ignore.case = TRUE, perl = TRUE)] <- "Art"
sp[grep("SOCJO", sp, ignore.case = TRUE, perl = TRUE)] <- "Sociology"
sp[grep("STOS$", sp, ignore.case = TRUE, perl = TRUE)] <- "International_Rel"
sp[grep("ZARZ$", sp, ignore.case = TRUE, perl = TRUE)] <- "Management"
...
data[, col] <- as.numeric(data[, col]) ### to get rid of non-numerical values
data[, col] <- enc2utf8(as.character(data[, col]))
data[, col][data[, col] == "1"] <- "<1k"
data[, col][data[, col] == "2"] <- "<1k-5k"
data[, col][data[, col] == "3"] <- "5k-20k"
data[, col][data[, col] == "4"] <- "20k-100k"
data[, col][data[, col] == "5"] <- ">100k"
data[, col] = factor(data[, col], ordered = TRUE,
                     levels = c("<1k", "<1k-5k", "5k-20k", "20k-100k", ">100k"))
